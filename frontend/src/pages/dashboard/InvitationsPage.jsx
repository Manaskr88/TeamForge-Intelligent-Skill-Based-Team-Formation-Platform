...
import { Mail, Check, X, Clock, Send } from 'lucide-react'
...
export default function InvitationsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === 'received' ? (
        received.length === 0 ? (
          <EmptyState icon={Mail} title="No pending invitations" description="When someone invites you to a team, it'll show up here." />
        ) : (
        ...
        sent.length === 0 ? (
          <EmptyState icon={Send} title="No sent invitations" description="Invite teammates from the Recommendations page." />
        ) : (
          <div className="space-y-3">
            {sent.map((inv, i) => (
              <motion.div key={inv._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                <div className="card p-5 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">
                      Invited to <span className="font-medium text-slate-700">{inv.team?.name || 'a team'}</span>
                    </p>
                    <p className="text-xs text-slate-400 mt-0.5">{new Date(inv.createdAt).toLocaleDateString()}</p>
                  </div>
