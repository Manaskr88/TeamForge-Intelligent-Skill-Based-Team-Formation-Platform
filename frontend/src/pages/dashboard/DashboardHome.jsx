          ...
          {recs.length === 0 ? (
            ...
            recs.map(({ user: u, compatibility }, i) => (
              <motion.div key={u._id} custom={i + 4} variants={cardVariants} initial="hidden" animate="visible">
                <Link to={`/dashboard/explore`} className="card p-4 flex items-center gap-3 group block">
                  <Avatar name={u.name} src={u.avatar} size="md" online={u.isOnline} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-semibold text-slate-900 truncate">{u.name}</p>
                      <span className="text-xs font-bold text-primary-600 shrink-0">{compatibility.score}%</span>
                    </div>
